...
import { MTGEvent, MTGSetEvent, CategoryKey } from './types';
import { Search } from 'lucide-react';

function App() {
  const { events, loading } = useEvents();
  const [selectedEvent, setSelectedEvent] = useState<MTGEvent | null>(null);
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedCategories, setSelectedCategories] = useState<Set<CategoryKey>>(
    new Set(['expansion', 'masters', 'secret_lair'])
  );
  const todayRef = useRef<HTMLDivElement>(null);
  const timelineRef = useRef<HTMLElement>(null);

  const { sortedEvents, nextSet, todayIndex } = useMemo(() => {
    const today = new Date().toISOString().split('T')[0];
    
    // Sort ascending (oldest on the left, newest on the right)
    const sorted = [...events].sort((a, b) => a.releaseDate.localeCompare(b.releaseDate));
    
    let next: MTGSetEvent | null = null;
    
    // Find next major set for Hero (irrespective of active filters)
    for (const e of sorted) {
      if (e.type === 'set' && e.releaseDate > today && !next) {
        next = e as MTGSetEvent;
        break;
      }
    }

    // Filter by active categories
    const categoryFiltered = sorted.filter(e => selectedCategories.has(e.category));
    
    let idx = categoryFiltered.length;
    for (let i = 0; i < categoryFiltered.length; i++) {
      if (categoryFiltered[i].releaseDate > today) {
        idx = i;
        break;
      }
    }
  ...
  const filteredEvents = useMemo(() => {
    ...
    return sortedEvents.filter(event => 
      ...
      event.releaseDate.includes(lowerQ)
    );
  ...
  }, [loading, filteredEvents.length]); // Scroll on initial load, search clear, or category change
      ...
      <main ref={timelineRef} className="relative w-full overflow-x-auto hide-scrollbar pt-8 pb-16 flex-1">
        <div className="flex flex-row items-center px-4 sm:px-[10vw] relative z-10 w-max min-w-full h-[500px]">
          ...
          {filteredEvents.map((event, index) => {
            const isPast = event.releaseDate <= todayDateIso;
            
            // Insert TODAY marker if this is the first upcoming event
          ...
          {/* Handle edge case where TODAY is after the very last event */}
          {!searchQuery && todayIndex === filteredEvents.length && (
            <div ref={todayRef} className="flex flex-col items-center justify-center shrink-0 px-4 h-full relative z-20">
              <div className="w-1 h-full bg-mtg-red self-center relative flex justify-center shadow-[0_0_15px_rgba(211,32,42,0.5)]">
                 <div className="absolute top-1/2 -translate-y-1/2 bg-mtg-black-bg border-2 border-mtg-red px-4 py-2 rounded text-mtg-red font-bold tracking-widest text-sm flex flex-col items-center whitespace-nowrap shadow-[0_0_10px_rgba(211,32,42,0.3)] z-30">
                   <span>TODAY</span>
